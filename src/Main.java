...
public class Main {
    public static void main(String[] args) {
        ...
        Cars toyota = new Cars("Toyota Camry", 2021, 2400, "красный", 2.4, 220);
        Cars volswagen = new Cars("VolsWagen Polo", 2022, 2000, "серый", 1.8, 190);
        Cars lexus = new Cars("Lexus RX", 2018, 3500, "белый", 3.5, 250);
        Cars mazda = new Cars("Mazda 6", 2019, 1700, "белый", 2.0, 200);
    ...
    public static void race(List<Cars> cars, String user_choice, Double distance, Random rnd) {
        for (Cars car : cars) {
            if (Objects.equals(car.model, "Toyota Camry")) car.max_speed = rnd.nextDouble(160, 220);
            if (Objects.equals(car.model, "VolsWagen Polo")) car.max_speed = rnd.nextInt(160, 190);
            if (Objects.equals(car.model, "Lexus RX")) car.max_speed = rnd.nextInt(160, 250);
            if (Objects.equals(car.model, "Mazda 6")) car.max_speed = rnd.nextInt(160, 200);
        }
        TreeMap<Double, Cars> timeLap = countTime(cars, distance);
        int count = 1;
        for(Map.Entry<Double, Cars> entry: timeLap.entrySet()) {
            System.out.println(count + ". " + entry.getValue().color + " " + entry.getValue().model
                    + " прошла дистанцию за " + entry.getKey() + " минут (максимальная скорость на трассе = " +
                    round_dbl(entry.getValue().max_speed) + "км/час);");
            count++;
        }
        if (Objects.equals(timeLap.firstEntry().getValue().model, user_choice)) System.out.println(
    ...
    public static TreeMap<Double, Cars> countTime(List<Cars> cars, Double distance) {
        ...
        for (Cars car: cars) {
            timeLap.put(round_dbl(distance / car.max_speed * 60), car);
        }
        return timeLap;
    }
